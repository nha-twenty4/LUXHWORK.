...
function makePlaceholderSvg(label: string, seed = 0) {
  const safeLabel = (label || "LUXH Works").replace(/&/g, "and").replace(/[^a-zA-Z0-9 _-]/g, " ").trim() || "LUXH Works";
  const palette = [
...
function resolveImageSource(source: string | undefined | null) {
  ...
  if (source.startsWith("http://") || source.startsWith("https://")) {
    ...
    if (resolved.includes("manus.space") || resolved.includes("manuspre.computer") || resolved.includes("manuscomputer.ai") || resolved.includes("manusvm.computer")) {
  ...
  if (source.startsWith("/manus-storage/")) {
    const base = storageBaseUrl || (typeof window !== "undefined" ? window.location.origin : "");
    return `${base}${source}`;
  }
  return source;
}
...
const profileImage = (name: string) => profileProjectMedia[name] ?? makePlaceholderSvg(name);

// Use hosted media URLs so the permanent site does not depend on another project's storage namespace.
const companyImages = {
  bar: profileImage("image87.png"), jewellery: profileImage("image88.jpeg"), jewelleryWide: profileImage("image89.jpeg"), retailPanels: profileImage("image90.jpeg"),
  visaOffice: profileImage("image37.jpeg"), visaReception: profileImage("image38.png"), visaDetail: profileImage("image39.jpeg"), warmInterior: profileImage("image42.jpeg"),
  warmDining: profileImage("image56.jpeg"), warmLounge: profileImage("image57.jpeg"), restaurant: profileImage("image58.jpeg"), restaurantDetail: profileImage("image59.jpeg"),
  fnb: profileImage("image60.jpeg"), retailStore: profileImage("image92.jpeg"), retailFront: profileImage("image93.jpeg"), retailInterior: profileImage("image94.jpeg"),
  residence: profileImage("image137.png"), residenceExterior: profileImage("image138.png"), residenceLounge: profileImage("image139.png"), residenceKitchen: profileImage("image140.png"),
};
...
const companyProjectImageSets: Record<string, string[]> = {
  ...
  "seascape-house": ["image56.jpeg", "image57.jpeg", "image32.jpeg", "image58.jpeg", "image59.jpeg", "image60.jpeg", "image61.jpeg"].map(profileImage),
  ...
  "atelier-common": ["image81.png", "image82.jpeg", "image83.jpeg", "image84.jpeg", "image85.jpeg", "image86.jpeg"].map(profileImage),
  "lukfook-sihanouk": ["image87.png", "image88.jpeg", "image89.jpeg", "image90.jpeg", "image91.jpeg"].map(profileImage),
  "lukfook-funmall": ["image92.jpeg", "image93.jpeg", "image94.jpeg", "image95.jpeg"].map(profileImage),
  "courtyard-study": ["image96.png", "image97.png", "image98.png", "image99.png", "image100.png"].map(profileImage),
  davidoff: ["image33.jpeg", "image62.jpeg", "image63.jpeg", "image64.png", "image65.jpeg"].map(profileImage),
  ...
  "lukfook-chipmong": ["image117.png", "image118.png", "image119.png", "image120.png", "image121.png"].map(profileImage),
  ...
  "pool-side-house": ["image137.png", "image138.png", "image139.png", "image140.png", "image141.png"].map(profileImage),
  "thailand-lukfook-rama9": ["image147.jpeg", "image148.jpeg", "image149.jpeg", "image150.jpeg", "image151.jpeg"].map(profileImage),
  "thailand-zhou-liufu": ["image152.jpeg", "image146.jpeg", "image4.jpeg", "image22.jpeg", "image153.jpeg"].map(profileImage),
  "thailand-lukfook-pinklao": ["image145.png", "image154.png", "image155.png", "image156.png", "image157.png"].map(profileImage),
};

const companyProjectDetails: Record<string, { title: string; category: Project["category"]; location: string; client: string; description: string; note: string; budget?: string }> = {
  "house-14": { title: "VISA WORLDWIDE BRANCH OFFICE EXPANSION (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "Visa Worldwide", description: "To propose corporate interior design and fit-out works for the Visa Phnom Penh office relocation.", note: "Corporate office space · 135 SQM" },
  "mori-residence": { title: "PP LINK SECURITY OFFICE (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "PP Link Security", description: "To propose an interior design concept for the PP Link Security office and broadcast station.", note: "Security office · Interior design concept" },
  northpoint: { title: "GOLDEN GROUP (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "Golden Group", description: "To propose corporate interior design and fit-out works for the Golden Group private lounge.", note: "Corporate private lounge · 170 SQM" },
  "seascape-house": { title: "RYUKO OMAKASE (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "Ryuko Omakase", description: "Interior design and fit-out works for Ryuko Omakase, shaped around a warm hospitality experience.", note: "Hospitality / F&B" },
  "frame-house": { title: "LUCKY BURGER TAKMAO", category: "Interior", location: "Takmao, Cambodia", client: "Lucky Burger", description: "Interior and facade fit-out work for the Lucky Burger Takmao restaurant.", note: "Restaurant / F&B · Interior and facade fit-out" },
  "field-notes": { title: "COMBI (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "Combi", description: "To propose interior design and fit-out works for the Combi retail store relocation.", note: "Retail shop · Outlet 1 & 2 · 130 SQM" },
  "atelier-common": { title: "LUK FOOK JEWELLERY AEON 3", category: "Interior", location: "Phnom Penh, Cambodia", client: "LUK FOOK Jewellery", description: "To propose fit-out works for the LUK FOOK Jewellery store at AEON 3. LUXHWORK exclusive.", note: "Retail jewellery · Fit-out works" },
  "courtyard-study": { title: "ONE OASIS WELLNESS SPA (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "One Oasis Wellness Spa", description: "To propose interior design and fit-out works for One Oasis Wellness Spa.", note: "Hospitality / leisure · 150 SQM" },
  davidoff: { title: "DAVIDOFF OF GENEVA (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "Davidoff of Geneva", description: "Interior design and fit-out work for the Davidoff of Geneva project in Phnom Penh.", note: "Retail / hospitality interior" },
  "the-hynd-hotel": { title: "SH HOTEL (PHNOM PENH)", category: "Interior", location: "Phnom Penh, Cambodia", client: "SH Hotel", description: "Hospitality interiors shaped around a calm, practical guest experience in Phnom Penh.", note: "Hospitality · 12 floors · 750 SQM" },
  "chj-jewellry-cb1": { title: "CHJ JEWELLRY CB1 AEON MALL 1", category: "Interior", location: "Phnom Penh, Cambodia", client: "CHJ Jewellery", description: "To propose fit-out works for the CHJ Jewellery store at AEON Mall 1.", note: "Retail jewellery · 58 SQM" },
};
...
function useColorPreset() {
  ...
  useEffect(() => window.localStorage.setItem("luxh-color-preset", preset), [preset]);
  return [preset, setPreset] as const;
}

function imagePresetClass(category: Project["category"], preset: ColorPreset = "auto") {
...
const projects: Project[] = [
  {
    ...
    description: "To propose corporate interior design and fit-out works for the Visa Phnom Penh office relocation.",
  ...
  },
  {
    ...
    description: "To propose an interior design concept for the PP Link Security office and broadcast station.",
  ...
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "Golden Group",
    description: "To propose corporate interior design and fit-out works for the Golden Group private lounge.",
  ...
  },
  {
    ...
    description: "Interior design and fit-out works for Ryuko Omakase, shaped around a warm hospitality experience.",
  ...
  },
  {
    ...
    client: "Lucky Burger",
    description: "Interior and facade fit-out work for the Lucky Burger Takmao restaurant.",
  ...
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "Combi",
    description: "To propose interior design and fit-out works for the Combi retail store relocation.",
  ...
  },
  {
    ...
    description: "To propose fit-out works for the LUK FOOK Jewellery store at AEON 3. LUXHWORK exclusive.",
  ...
  },
  {
    ...
    description: "To propose interior design and fit-out works for One Oasis Wellness Spa.",
  ...
  },
  {
    ...
    description: "Interior design and fit-out work for the Davidoff of Geneva project in Phnom Penh.",
    note: "Retail / hospitality interior",
    size: "wide",
    gallery: [companyImages.jewelleryWide, companyImages.jewellery, companyImages.retailInterior],
    floorPlan: "",
    pdf: "",
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "SH Hotel",
    description: "Hospitality interiors shaped around a calm, practical guest experience in Phnom Penh.",
    note: "Hospitality · 12 floors · 750 SQM",
    size: "tall",
    gallery: [companyImages.warmLounge, companyImages.restaurant, companyImages.warmDining],
    floorPlan: "",
    pdf: "",
  },
  {
    ...
    description: "To propose fit-out works for the CHJ Jewellery store at AEON Mall 1.",
    note: "Retail jewellery · 58 SQM",
    size: "standard",
    gallery: [companyImages.retailPanels, companyImages.retailFront, companyImages.jewellery],
    floorPlan: "",
    pdf: "",
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "Visa Worldwide",
    description: "Corporate interior design and fit-out works for the Visa Phnom Penh branch office.",
    note: "Corporate office space",
    size: "wide",
    gallery: ["image40.jpeg", "image46.jpeg", "image47.jpeg", "image48.jpeg", "image49.jpeg", "image50.jpeg"].map(profileImage),
    floorPlan: "",
    pdf: "",
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "PP Link Security",
    description: "Interior design concept for the PP Link Security broadcast station.",
  ...
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "LUK FOOK Jewellery",
    description: "Fit-out works for the LUK FOOK Jewellery store at Preah Sihanouk Boulevard.",
  ...
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "LUK FOOK Jewellery",
    description: "Fit-out works for the LUK FOOK Jewellery store at Funmall TK.",
    note: "Retail jewellery · LUXHWORK exclusive",
    size: "standard",
  ...
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "LUK FOOK Jewellery",
    description: "Fit-out works for the LUK FOOK Jewellery store at Chipmong Mega Mall 271.",
    note: "Retail jewellery · LUXHWORK exclusive",
    size: "wide",
  ...
  },
  {
    ...
    location: "Phnom Penh, Cambodia",
    client: "Lao Miao",
    description: "Fit-out works for the Lao Miao Jewellery store at CBF-Naga 2.",
  ...
  {
    slug: "pool-side-house",
    title: "POOL SIDE HOUSE RENOVATION AT SIEM REAP",
    category: "Architecture",
    image: companyProjectImageSets["pool-side-house"][0],
    year: "Not listed",
    location: "Siem Reap, Cambodia",
    client: "Private residential client",
    description: "Conceptual draft and floor layout for a pool-side house renovation at Siem Reap.",
    note: "Residential renovation · Conceptual draft",
    size: "wide",
    gallery: companyProjectImageSets["pool-side-house"],
    floorPlan: "",
    pdf: "",
  },
  {
    ...
    description: "Fit-out works for the LUKFOOK Jewellery store at Central-Rama9.",
  ...
  },
  {
    ...
    description: "Fit-out works for the Zhou Liufu Jewelry store at Central Rama 9.",
    note: "Thailand project · Retail jewellery",
    size: "standard",
  ...
  },
  {
    ...
    location: "Bangkok, Thailand",
    client: "LUKFOOK Jewelry",
    description: "Fit-out works for the LUKFOOK Jewellery store at Central-Pinklao.",
    note: "Thailand project · Retail jewellery",
    size: "tall",
  ...
  },
  {
    ...
    description: "Interior design and fit-out works for the Ratanac Mealea jewellery retail stores.",
  ...
  },
  {
    ...
    description: "Commercial interior design for the Fabric Factory project in Phnom Penh.",
...
function mapPersistedProject(item: PersistedProject): Project {
  ...
  const gallery = companyGallery ?? item.gallery.filter((asset) => asset.kind === "gallery").map((asset) => asset.url);
...
function usePortfolioProjects() {
  ...
  const correctedStaticProjects = projects.map((project) => {
    ...
    return companyGallery ? { ...project, image: companyGallery[0], gallery: companyGallery } : project;
  });
  const mergedProjects = persistedProjects.length
    ? [...persistedProjects, ...correctedStaticProjects.filter((project) => !persistedSlugs.has(project.slug))]
...
const serviceDetailCopy: Record<string, { headline: string; body: string; tags: string; scope?: string[] }> = {
  "interior-design-and-consultancy": { headline: "Spaces shaped with clarity.", body: "Tailored interior solutions that bring together refined design, functionality, and purpose — thoughtfully shaped around each client’s identity, needs, and way of working.", tags: "SPATIAL DIRECTION / DESIGN DEVELOPMENT / TECHNICAL & COST", scope: [
...
const faqItems = [
  ["What types of commercial projects does LUXHWORK undertake?", "Offices, retail, F&B, hospitality, showrooms, jewellery stores, kiosks and commercial renovations."],
  ["Can you provide design consultancy without construction?", "Yes. We can support design and consultancy as a standalone scope, or continue through fit-out and delivery when the project requires it."],
  ["Can LUXHWORK manage the complete design-and-fit-out process?", "Yes. Our capabilities cover design, cost direction, fit-out coordination, project management, MEP coordination and handover support."],
  ["How early should we engage you before opening?", "As early as possible. An initial brief or feasibility review helps clarify the site, programme, budget and approvals before major commitments are made."],
  ["How is the project budget developed and controlled?", "We establish the brief and scope first, then develop design, cost direction and a priced BOQ. Programme, cost and reporting are reviewed through delivery."],
  ["Do you work in Thailand as well as Cambodia?", "Yes. LUXHWORK is presented as serving Cambodia and Thailand, with local execution and regional standards."],
  ["What happens after I send an enquiry?", "We review the location, approximate area, target opening date and business need, then recommend the right next step: a site visit, feasibility review or design proposal."],
  ["Do you work with an existing landlord, consultant or contractor team?", "Yes. The project scope can be shaped around the existing team, with LUXHWORK coordinating the design, fit-out, reporting or specialist layers that are needed."],
  ["Can you support a project after opening?", "Yes. Aftercare and continued operational support are part of the stated process, including repair, alteration and support for the space."],
  ["What information should I prepare before our first conversation?", "Please share the location, approximate area, target opening date, project type and what the business needs the space to achieve. An approximate budget is also helpful."],
] as const;

function FAQSection() {
  const [openIndex, setOpenIndex] = useState<number | null>(null);
  return <section className="faq-section" id="faq"><div className="faq-heading"><SectionLabel number="03">Project questions</SectionLabel><h2>Questions worth answering<br /><em>before building.</em></h2></div><div className="faq-list">{faqItems.map(([question, answer], index) => <div className={`faq-item${openIndex === index ? " faq-item-open" : ""}`} key={question}><button type="button" className="faq-question" aria-expanded={openIndex === index} onClick={() => setOpenIndex(openIndex === index ? null : index)}><span>{question}</span><strong>{openIndex === index ? "−" : "+"}</strong></button><div className="faq-answer"><p>{answer}</p></div></div>)}</div></section>;
}

function CookieConsent() {
  const [visible, setVisible] = useState(false);
  useEffect(() => { setVisible(localStorage.getItem("luxh-cookie-consent") === null); }, []);
  const choose = (value: "accepted" | "declined") => { localStorage.setItem("luxh-cookie-consent", value); setVisible(false); window.dispatchEvent(new CustomEvent("luxh-cookie-consent", { detail: value })); };
  if (!visible) return null;
  return <aside className="cookie-consent" role="dialog" aria-label="Cookie consent"><div><strong>Cookies, with care.</strong><p>Necessary cookies keep this site working. Optional analytics only starts when you accept.</p></div><div className="cookie-actions"><button type="button" onClick={() => choose("declined")}>Decline</button><button type="button" onClick={() => choose("accepted")}>Accept analytics</button></div></aside>;
}
...
function ProjectInquiryForm({ projectTitle }: { projectTitle: string }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const values = new FormData(event.currentTarget); const name = String(values.get("name") ?? "").trim(); const email = String(values.get("email") ?? "").trim(); const details = String(values.get("details") ?? "").trim();
    if (name.length < 2 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || details.length < 20) { setStatus("Please provide your name, a valid email and at least 20 characters about the project."); return; }
    try { await submitInquiry.mutateAsync({ name, email, phone: String(values.get("phone") ?? "") || undefined, service: projectTitle, details, budget: String(values.get("budget") ?? "") || undefined, timeline: String(values.get("timeline") ?? "") || undefined }); event.currentTarget.reset(); setStatus("Thank you — we’ll be in touch shortly."); } catch { setStatus("We could not send the enquiry. Please try again or email ADMIN@LUXHWORK.COM."); }
  };
  return <section className="project-inquiry"><div><SectionLabel number="Project enquiry">Start with SH Hotel</SectionLabel><h2>Have a similar<br /><em>space in mind?</em></h2><p>Tell us about the location, brief and timing. We will help define the right next step.</p></div><form className="contact-form" onSubmit={submit}><div className="form-split"><label>Name *<input name="name" required placeholder="Your name" /></label><label>Email *<input name="email" type="email" required placeholder="you@company.com" /></label><label>Phone or WhatsApp<input name="phone" placeholder="+855 ..." /></label><label>Budget / timeline<input name="timeline" placeholder="Optional" /></label></div><label>Project details *<textarea name="details" required minLength={20} rows={5} placeholder="Tell us about the space or brief..." /></label>{status && <p className="form-error" role="status">{status}</p>}<p className="privacy-consent">By sending this enquiry you agree that LUXHWORK may contact you about your project.</p><button className="button button-dark" type="submit" disabled={submitInquiry.isPending}>{submitInquiry.isPending ? "Sending…" : "Discuss your project"} <ArrowUpRight size={17} /></button></form></section>;
}
...
export function PageShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const cleanups = buttons.map((button) => {
      ...
      const onLeave = () => { button.style.transform = "translate3d(0, 0, 0)"; };
      button.addEventListener("pointermove", onMove);
      button.addEventListener("pointerleave", onLeave);
      return () => { button.removeEventListener("pointermove", onMove); button.removeEventListener("pointerleave", onLeave); };
    });
    return () => cleanups.forEach((cleanup) => cleanup());
  }, [location]);
  useEffect(() => {
    const measurementId = import.meta.env.VITE_GA_MEASUREMENT_ID as string | undefined;
    if (!measurementId || localStorage.getItem("luxh-cookie-consent") !== "accepted") return;
    if (document.querySelector("script[data-luxh-ga]")) return;
    const script = document.createElement("script"); script.async = true; script.src = `https://www.googletagmanager.com/gtag/js?id=${measurementId}`; script.dataset.luxhGa = "true"; document.head.appendChild(script);
  ...
  return (
    <div className="site-shell">
      <Header />
      <main key={location} className={`page-transition ${location.startsWith("/projects/") ? "project-detail-transition" : ""}`}>{children}</main>
      <Footer />
      <CookieConsent />
    </div>
  );
...
function Header() {
  ...
  return (
    <header className={`site-header ${isHomeTop ? "header-on-hero" : "header-solid"} ${scrolled ? "header-scrolled" : ""} theme-${theme}`}>
      <div className="header-inner">
        <a href="/" className="brand" aria-label="LUXH Works home" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
        ...
        <button className="menu-toggle" onClick={() => setOpen(!open)} aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} aria-controls="mobile-navigation">
      ...
      <div id="mobile-navigation" className={`mobile-nav ${open ? "mobile-nav-open" : ""}`} role="dialog" aria-label="Mobile navigation" aria-hidden={!open}>
        ...
        <a className="mobile-whatsapp" href="https://wa.me/85589900300" target="_blank" rel="noreferrer"><span aria-hidden="true" /> WhatsApp us</a>

        <div className="mobile-nav-footer">ADMIN@LUXHWORK.COM<br />+855 89 900 300</div>
    ...
    <footer className="footer">
      <div className="footer-top">
        <div className="footer-brand-block">
          <Link href="/" className="footer-wordmark" aria-label="LUXHWORK home" style={{fontFamily: 'MOMCAKE'}}>LUXHWORK</Link>
          <p>A commercial interior consultancy and fit-out company serving businesses in Cambodia and Thailand.</p>
        </div>
        <div className="footer-links">
          <span className="footer-kicker">Contact</span>
          <a className="footer-contact" href="tel:+85589900300">+855 89 900 300 <ArrowUpRight size={13} /></a>
          <a className="footer-contact" href="mailto:ADMIN@LUXHWORK.COM">ADMIN@LUXHWORK.COM <ArrowUpRight size={13} /></a>
        </div>
        <div className="footer-links">
          <span className="footer-kicker">Connect</span>
          <a className="footer-contact" href="https://t.me/+85589900300" target="_blank" rel="noreferrer">Telegram <ArrowUpRight size={13} /></a>
          <a className="footer-contact" href="https://wa.me/85589900300" target="_blank" rel="noreferrer">WhatsApp <ArrowUpRight size={13} /></a>
      ...
      <div className="footer-nap">
        <div><span className="footer-kicker">Studio</span><strong style={{ fontFamily: "MOMCAKE", fontSize: "16px" }}>LUXHWORK</strong><a className="footer-address" href="https://maps.app.goo.gl/LBirY3B6T78iRpFeA?g_st=ic" target="_blank" rel="noreferrer">#MF, No. 112G E0, Preah Ang Yukanthor Street (19), Sangkat Phsar Kandal 2, Khan Daun Penh, Phnom Penh 12205, Cambodia <ArrowUpRight size={12} /></a></div>
        <div><span className="footer-kicker">Direct</span><a href="tel:+85589900300" style={{ fontSize: "13px" }}>+855 89 900 300</a><a href="mailto:admin@luxhwork.com" style={{ fontSize: "13px" }}>admin@luxhwork.com</a></div>
        <div><span className="footer-kicker">Hours</span><p style={{ fontSize: "12px" }}>Monday–Friday 08:30–17:30<br />Saturday 08:30–13:00</p></div>
      </div>
      <div className="footer-bottom"><span>© 2026 LUXHWORK</span><span>Phnom Penh City, Cambodia</span><Link href="/privacy">Privacy policy</Link></div>
    </footer>
  );
}
...
export function Home() {
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => updateImagePosition(event.clientX, event.clientY);
    const handleTouchMove = (event: TouchEvent) => {
      const touch = event.touches[0];
      if (touch) updateImagePosition(touch.clientX, touch.clientY);
    };
    const resetImage = () => { image.style.transform = "perspective(900px) rotateX(0) rotateY(0) scale(1) translate3d(0, 0, 0)"; };
  ...
  useEffect(() => {
    const revealTargets = Array.from(document.querySelectorAll<HTMLElement>(".text-reveal, .service-row-reveal"));
    if (!("IntersectionObserver" in window)) {
    ...
    <PageShell>
      <section className="hero" id="hero" ref={heroRef}>
        ...
        <div className="hero-copy">
          ...
          <h1 className="hero-title" aria-label="Commercial spaces designed to perform.">
            <span className="hero-title-line"><span>Commercial spaces</span></span>
      ...
      <section className="manifesto" id="process">
        ...
        <div className="v2-process-header">
          ...
          <p className="manifesto-copy">Each stage closes a different risk before it reaches your budget.</p>
        </div>
        <div className="v2-process-steps">
          <div><span>01</span><strong>BRIEF &amp; CONSULT</strong><p>Define users, operation, priorities and constraints.</p></div>
          <div><span>02</span><strong>DESIGN &amp; COST</strong><p>Develop concept, technical drawings and priced BOQ.</p></div>
          <div><span>03</span><strong>BUILD &amp; MANAGE</strong><p>Coordinate site works, contractors, inspections and reporting.</p></div>
          <div><span>04</span><strong>HANDOVER &amp; CARE</strong><p>Commission, inspect, hand over and support the space.</p></div>
          <div><span>05</span><strong>MAINTAIN</strong><p>Repair, alter and support continued operation.</p></div>
        </div>
      </section>
...
export function ProjectDetailPage() {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("keydown", onKeyDown); window.removeEventListener("touchstart", onTouchStart); window.removeEventListener("touchend", onTouchEnd); };
...
export function ServiceDetailPage() {
  const { slug } = useParams<{ slug: string }>();
  const service = services.find((item) => serviceSlug(item.title) === slug) ?? services[0];
  ...
  return (
    <PageShell>
      <section className="page-hero services-page-hero">
        ...
        <h1>Shape the idea<br /><em>into a place.</em></h1>
        <p className="page-hero-copy">Five connected capabilities, brought together with care from first thought to final frame.</p>
      </section>
      <section className="services-page-list" id="capabilities">
        <SectionLabel number="01 – 05">Capabilities / One partner across every project layer.</SectionLabel>
        <div className="services-page-grid">
          {services.map((service) => { const ServiceIcon = service.icon; return <article className="services-page-card" key={service.number}><div className="services-page-card-top"><span>{service.number}</span><span>Available for new projects</span></div><div className="services-page-marker" aria-hidden="true"><ServiceIcon size={24} strokeWidth={1.4} /></div><h2>{service.title}</h2><p>{service.text}</p><ul>{service.items.map((item) => <li key={item}>{item}</li>)}</ul><Link href={`/services/${serviceSlug(service.title)}`} className="text-link">View <ArrowUpRight size={16} /></Link></article>; })}
        </div>
      </section>
      <section className="services-process" id="process"><SectionLabel number="02">Process / From first thought to final frame</SectionLabel><div className="services-process-grid"><div><span>01</span><h3>Listen &amp; define</h3><p>Brief, site, business objectives and the decisions that matter most.</p></div><div><span>02</span><h3>Design &amp; test</h3><p>Concept, material direction, technical thinking and cost control.</p></div><div><span>03</span><h3>Build &amp; coordinate</h3><p>Fit-out, MEP, suppliers and programme brought into one clear rhythm.</p></div><div><span>04</span><h3>Open &amp; support</h3><p>Handover, aftercare and continued support once the space is working.</p></div></div><Link href="/contact#faq" className="text-link">Read the 10 project FAQs <ArrowUpRight size={16} /></Link></section>
      <section className="services-page-cta"><p>Have a brief in mind?</p><h2>Let’s find the<br /><em>right starting point.</em></h2><Link href="/contact" className="button button-light">Start a conversation <ArrowUpRight size={17} /></Link></section>
    </PageShell>
  );
}
...
function ContactContent() {
  ...
  const submitForm = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (details.length < 20) { setFormError("Please tell us a little more about the project (at least 20 characters)."); return; }
    setFormError("");
    try {
      await submitInquiry.mutateAsync({
        ...
        service,
        details: [details, bookingType && `Preferred consultation: ${bookingType}`, contactWindow && `Preferred contact window: ${contactWindow}`].filter(Boolean).join("\n\n"),
      ...
      const message = error instanceof Error ? error.message : "We could not save your inquiry. Please try again.";
  ...
  return (
    <>
      <section className="contact-hero" id="contact"><p className="eyebrow eyebrow-light">Contact / New projects and thoughtful conversations</p><h1>Let's work<br /><em>together.</em></h1><div className="contact-intro"><p>Tell us a little about what you are making. We usually reply within two working days.</p><a href="#booking" className="button button-light">Book a consultation <ArrowUpRight size={17} /></a></div><div className="contact-mark">LW</div></section>
      <section className="contact-main"><div className="contact-details contact-details-centered"><SectionLabel number="01">Get in touch / Visit by appointment</SectionLabel><div className="contact-detail-block"><span>Phone</span><a href="tel:+85589900300">+855 89 900 300</a></div><div className="contact-detail-block"><span>Studio</span><p style={{ fontSize: "22px" }}>#MF, NO. 112G EO, STREET 19, SANGKAT PHSAR KANDAL 2, KHAN DAUN PENH, PHNOM PENH, KINGDOM OF CAMBODIA</p></div><div className="contact-detail-block"><span>Email</span><a href="mailto:ADMIN@LUXHWORK.COM">ADMIN@LUXHWORK.COM</a></div><div className="contact-detail-block"><span>Quick contact</span><div className="social-row"><a href="https://t.me/+85589900300" target="_blank" rel="noreferrer">Telegram <ArrowUpRight size={13} /></a><a href="https://wa.me/85589900300" target="_blank" rel="noreferrer">WhatsApp <ArrowUpRight size={13} /></a></div></div><div className="contact-detail-block"><span>Elsewhere</span><div className="social-row"><a href="https://instagram.com" target="_blank" rel="noreferrer">Instagram <ArrowUpRight size={13} /></a><a href="https://facebook.com" target="_blank" rel="noreferrer">Facebook <ArrowUpRight size={13} /></a><a href="https://linkedin.com" target="_blank" rel="noreferrer">LinkedIn <ArrowUpRight size={13} /></a></div></div><div className="map-embed"><iframe src="https://www.google.com/maps?q=LUXHWORK%20CO.%2C%20LTD%2C%20MF%2C%20No.112G%20E0%2C%20Preah%20Ang%20Yukanthor%20Street%20(19)%2C%20Phnom%20Penh%2C%20Cambodia&output=embed" width="600" height="450" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="strict-origin-when-cross-origin" title="LUXHWORK CO., LTD map" /></div></div>
        <form className="contact-form contact-enquiry-form" id="booking" onSubmit={submitForm}><div className="form-intro"><span>02</span><h2>Book a project consultation</h2><p>Share the brief and we’ll arrange a practical first conversation about scope, timing and next steps.</p></div><div className="form-split"><label>Name *<input name="name" required placeholder="Your name" /></label><label>Company<input name="company" placeholder="Company name" /></label><label>Email *<input name="email" type="email" required placeholder="you@company.com" /></label><label>Phone or WhatsApp *<input name="phone" required placeholder="+855 ..." /></label><label>Project type *<select name="service" required defaultValue=""><option value="" disabled>Select a service</option>{services.map((service) => <option value={service.title} key={service.number}>{service.title}</option>)}</select></label><label>Location *<input name="location" required placeholder="City / country" /></label><label>Approximate area (sqm)<input name="area" inputMode="numeric" placeholder="e.g. 300" /></label><label>Target opening date<input name="opening" placeholder="Month / year" /></label><label>Preferred consultation<select name="bookingType" defaultValue=""><option value="">Choose an option</option><option value="Video call">Video call</option><option value="Studio meeting">Studio meeting</option><option value="Site visit">Site visit</option></select></label><label>Preferred contact window<input name="contactWindow" placeholder="e.g. weekday mornings" /></label></div><label>What does the space need to achieve? *<textarea name="details" required minLength={20} rows={5} placeholder="Tell us about the business, space or brief..." /></label>{formError && <p className="form-error" role="alert">{formError}</p>}<p className="privacy-consent">By submitting this form you agree that LUXHWORK may contact you about your enquiry. See our Privacy Policy.</p><button type="submit" className="button button-dark" disabled={submitInquiry.isPending}>{submitInquiry.isPending ? "Sending…" : "Request consultation"} <ArrowUpRight size={17} /></button></form>
      </section>
      <FAQSection />
    </>
  );
